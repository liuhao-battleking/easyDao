...
<div id="main" class="wrap">
	...
	<div class="main">
		<h2>修改商品</h2>
		<div class="manage">
			<form action="../proServlet?type=update&pid=${product.productid } " method="post">
				<table class="form">
					<tr>
					...
					<tr>
						<td class="field">所属分类：${product.cateid }  ${product.cateid%3==0} </td>
						<td>
							<select name="cateid">
							
							
							    <option value="0" <c:if test="${product.cateid==0}"> selected</c:if> >未分类</option>
								<option value="1" <c:if test="${product.cateid==1}"> selected</c:if> >电器</option>
								<option value="2" <c:if test="${product.cateid==2}"> selected</c:if> >服饰</option>
